      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        {!loading && totalUserFound !== null &&(
          <div className="bg-gray-800 p-3 rounded border border-gray-700 text-sm text-gray-300">
            found <span className="text-blue-400 font-bold">{totalUserFound.toLocaleString()}</span> {" "}
            user with the name <span className="italic">{debounceTerm}</span>
          </div>
        )}
        {loading && (
          <p className="text-center text-gray-400">Finding your account</p>
        )}
        {!loading && userProfile && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2">
              <RepositoryCard repos={userRepo} currentPage={page} onPageChange={setPage} totalRepos={userProfile.public_repos}/>
            </div>
          </div>
        )}
      </div>
